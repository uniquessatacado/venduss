...
const CategoryCard: React.FC<{ item: Category; onClick?: () => void; isSelected?: boolean; isDark: boolean }> = ({ item, onClick, isSelected, isDark }) => {
  ...
  return (
    <div className="group cursor-pointer snap-start flex-shrink-0 w-[72px] md:w-24 flex flex-col items-center gap-2" onClick={onClick}>
      {/* Container do Anel Estilo Instagram */}
      <div className={`relative p-[3px] rounded-full transition-all duration-300 ${isSelected 
          ? 'bg-gradient-to-tr from-pink-500 via-purple-500 to-yellow-500 shadow-lg shadow-purple-500/20 scale-105' 
          : 'bg-gradient-to-tr from-gray-200 to-gray-300 hover:from-pink-400 hover:to-purple-400'
        }`}>
        
        {/* Borda interna branca para separar a foto do anel */}
      ...
      <div className="text-center px-1 max-w-full">
        <h3 className={`font-medium text-[11px] md:text-xs tracking-tight truncate w-full ${isSelected ? 'text-black font-bold' : 'text-gray-600 group-hover:text-black'}`}>
          {item.name}
        </h3>
      </div>
    </div>
  ...
  return (
    <div className={`py-4 ${isDark ? 'bg-black' : 'bg-white'}`}>
       <div className="max-w-7xl mx-auto pl-4 sm:pl-6 lg:pl-8 mb-3">
         <h2 className={`text-sm font-bold uppercase tracking-widest ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>
            Categorias
         </h2>
       </div>
      <div 
        className="flex overflow-x-auto gap-4 px-4 sm:px-6 lg:px-8 no-scrollbar pb-4 snap-x"
      >
        {categories.map((item) => (
          <CategoryCard 
            key={item.id} 
